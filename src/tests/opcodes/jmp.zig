...
test "JMP (Indirect) jumps to incorrect address if vector is on a page boundary" {
    var mem: Mem = Mem{};
    var cpu: Cpu = Cpu{};
    ...
    mem.setMemory(.{
        .{ .start_address = 0xFFFC, .bytes = .{0x0200} },
        .{ .start_address = 0x0200, .bytes = .{ @intFromEnum(Cpu.Opcode.jmp_indirect), 0xA3FF } },
        .{ .start_address = 0xA3FF, .bytes = .{0x80AA} },
    });
    cpu.reset(&mem);
    var expected_cpu = cpu;
    ...
    // then:
    expected_cpu.PC = 0xAA; // bcz little endian, stored as 0xA3FF = 0xAA, 0xA400 = 0x80
    try std.testing.expectEqual(@as(Cpu.ExecuteError!void, void{}), result);
    try std.testing.expectEqualDeep(expected_cpu, cpu);
}
